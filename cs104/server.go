...
import (
	"context"
	"crypto/tls"
	"github.com/tiezhong2004/iecp5/asdu"
	"github.com/tiezhong2004/iecp5/clog"
	"net"
	str "strings"
	"sync"
	"time"
...
func (sf *Server) ListenAndServer(addr string, authorizeClientIP []string) {
	...
	for {
		...
		clientPort := conn.RemoteAddr().String()
		client := str.Split(clientPort, ":")[0]
		okclient := false
		for _, ip := range authorizeClientIP {
			if ip == client {
				okclient = true
				sf.wg.Add(1)
				go func() {
					sess := &SrvSession{
						...
						handler:  sf.handler,
						conn:     conn,
						rcvASDU:  make(chan []byte, sf.config.RecvUnAckLimitW<<5),
						sendASDU: make(chan []byte, sf.config.SendUnAckLimitK<<7),
						rcvRaw:   make(chan []byte, sf.config.RecvUnAckLimitW<<6),
						sendRaw:  make(chan []byte, sf.config.SendUnAckLimitK<<8), // may not block!
		...
		if !okclient {
			conn.Close()
			sf.Warn("%s client disconnected!", client)
		}
	}
}
